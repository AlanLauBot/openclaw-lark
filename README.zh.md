...
> **本分支的定制说明**
> 在官方插件基础上，本分支主要面向 **以 Bot / tenant 身份运行的数字员工场景** 做了改造，而不是把 OpenClaw 仅作为“代某个用户操作”的个人助手。核心变化是：文档、Sheet、多维表等操作可以尽量以应用自身身份（`as: "tenant"`）发起，让创建、编辑、写入等行为归属到 Bot 本身，而不是某个具体用户；同时新增了 `feishu_doc_permission` 文档权限管理工具，用于给文档添加成员、修改权限和管理协作者。
>
> **为什么这样做**
> 在我们的使用场景中，OpenClaw 更像一个虚拟员工，权限开给应用本身，由它自主阅读、修改文档并沉淀工作结果。这样做的价值包括：
